...
describe('Legacy routes', () => {
  ...
  test('GET /api/reservations denies anonymous access', async () => {
    const response = await request(app).get('/api/reservations').query({ businessId: 'negocio1' });
  ...
  test('PUT /api/reservations/:id allows admin update', async () => {
    ...
    db.query
      .mockResolvedValueOnce({ rows: [{ business_id: 'negocio1' }] })
      .mockResolvedValueOnce({ rows: [{ id: 1, business_id: 'negocio1', disponibilidad: 'Confirmado', notas: 'ok' }] });
