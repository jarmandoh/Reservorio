      ...
      <div class="max-w-3xl mx-auto space-y-6">
        ...
        @if (loading()) {
          ...
          <section class="card p-5 flex items-center gap-4">
            <div
              class="w-14 h-14 rounded-2xl flex items-center justify-center bg-primary text-white font-display text-lg font-bold shrink-0"
            >
              {{ initials(c.name) }}
            </div>
            <div class="min-w-0">
          ...
          <section class="card p-5 space-y-3">
            ...
            <p class="text-sm text-on-surface-variant">
              Descarga una copia completa de tus datos o elimina tu cuenta (los datos se anonimizan de forma
              permanente). Consulta la
          ...
          @if (bookings().length === 0) {
            <section class="card p-8 flex flex-col items-center gap-3 text-center">
              ...
              <p class="text-sm text-on-surface-variant">
                Cuando reserves en algún negocio, tu historial aparecerá aquí.
              </p>
            ...
            <section class="flex flex-col gap-3">
              @for (b of bookings(); track b.id) {
                <article class="card p-4 sm:p-5">
                  ...
                  <button
                    class="btn-secondary btn-sm mt-4 flex items-center gap-1.5"
                    type="button"
                    (click)="bookAgain(b)"
                  >
            ...
            @if (total() != null && total()! > pageSize()) {
              <nav class="flex items-center justify-between gap-3" aria-label="Paginación del historial">
                <button
                  class="btn-tertiary btn-sm flex items-center gap-1.5"
                  type="button"
                ...
                <button
                  class="btn-tertiary btn-sm flex items-center gap-1.5"
                  type="button"
                  [disabled]="page() >= totalPages() || changingPage()"
                  (click)="goToPage(page() + 1)"
                >
                  Siguiente
...
export class CustomerHistoryComponent implements OnInit {
  ...
  loadHistory(page: number): void {
    ...
    this.api
      .getCustomerHistory(customerId, token, page, this.pageSize())
  ...
  exportData(): void {
    ...
    this.api
      .exportCustomerData(customerId, token)
      .pipe(takeUntilDestroyed(this.destroyRef))
      .subscribe({
        next: data => {
