...
3. Dentro de la página de tu negocio, selecciona la opción **"Administrar"** o accede directamente a `/business/<id-de-tu-negocio>/login`.
...
> Por seguridad, no compartas tu PIN con personas no autorizadas. El PIN se almacena como hash (bcrypt) y no puede recuperarse, solo restablecerse.
...
Sí, en el panel de tu negocio existe la sección de **generador de franjas** (bulk) para crear bloques recurrentes; también puedes crear franjas individuales con estado **Disponible** o contactar al administrador global.
